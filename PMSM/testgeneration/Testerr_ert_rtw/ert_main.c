...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Testerr_M->Timing.taskTime0;
  Testerr_step0();
  ...
  if (need2runFlags[1]) {
    ...
    {
     case 1 :
      currentTime = (extmodeSimulationTime_T) ((Testerr_M->Timing.clockTick1) *
        0.1);
      Testerr_step1();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(Testerr_M->extModeInfo, &rtmGetTFinal(Testerr_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Testerr_M, true);
    }
  }
