...
void rt_OneStep(void)
{
   ...
   */
  G431currentloop_SetEventsForThisBaseStep(eventFlags);
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) G431currentloop_M->Timing.taskTime0;
  G431currentloop_step0();
    ...
    switch (1)
    {
     case 1 :
      currentTime = (extmodeSimulationTime_T)
        ((G431currentloop_M->Timing.clockTick1) * 0.1);
      G431currentloop_step1();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(G431currentloop_M->extModeInfo, &rtmGetTFinal
    (G431currentloop_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(G431currentloop_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(G431currentloop_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
