...
void rt_OneStep(void)
{
   ...
   */
  PICurrentloop_SetEventsForThisBaseStep(eventFlags);
  __enable_irq();
  PICurrentloop_step0();
...
int main(int argc, char **argv)
{
  ...
  MX_ADC1_Init();
  MX_TIM1_Init();
  rtmSetErrorStatus(PICurrentloop_M, 0);
  PICurrentloop_configure_interrupts();
  PICurrentloop_initialize();
  __disable_irq();
  ARMCM_SysTick_Config(modelBaseRate);
  runModel =
    rtmGetErrorStatus(PICurrentloop_M) == (NULL);
  __enable_irq();
  __enable_irq();
  while (runModel) {
    stopRequested = !(
  ...
  ;
  PICurrentloop_unconfigure_interrupts();
